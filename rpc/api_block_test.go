...
func TestBlockWithTxHashes(t *testing.T) {
	...
	type testSetType struct {
		...
		ExpectedError             error
		ExpectedBlockWithTxHashes types.Block
	}
	...
	for _, test := range testSet {
		...
		if diff, err := spy.Compare(block, false); err != nil || diff != "FullMatch" {
			t.Fatal("expecting to match", err)
		}
		...
		if test.ExpectedBlockWithTxHashes.BlockHash.String() == "" {
			continue
		}
...
func TestBlockWithTxsAndInvokeTXNV0(t *testing.T) {
	...
	type testSetType struct {
		...
		ExpectedBlockWithTxs        types.Block
	}
	testSet := map[string][]testSetType{
		"mock": {},
		"testnet": {
	...
	for _, test := range testSet {
		...
		blockWithTxs, ok := blockWithTxsInterface.(*types.Block)
		if !ok {
		...
		if test.ExpectedBlockWithTxs.BlockHash.String() == "" {
			continue
		}
