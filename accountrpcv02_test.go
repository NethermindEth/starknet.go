...
func TestRPCv02AccountNonce(t *testing.T) {
...
func TestRPCv02AccountEstimateFee(t *testing.T) {
	testConfig := beforeRPCEach(t)
...
func TestRPCv02Account_Execute(t *testing.T) {
	...
	for _, test := range testSet {
		...
		defer cancel()
		status, err := account.rpcv02.WaitForTransaction(ctx, types.HexToHash(execute.TransactionHash), 8*time.Second)
		if err != nil {
