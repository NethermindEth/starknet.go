...
Gets the number (height) of the most recently accepted block on Starknet. The block number (also called block height) starts from 0 (genesis block) and increments with each new block. In rare cases, this method can return `ErrNoBlocks` if the chain has no blocks yet.
...
```
:::note
Create a `.env` file in your project root and install the godotenv package to tun the code snippet.
:::
## Error Handling
