...
BuildDeclareTxn creates a declare transaction for registering a new contract class on Starknet. It requires both the Sierra class (high-level) and the compiled CASM class hash (low-level executable).
...
- [contracts.UnmarshalCasmClass](/docs/contracts/functions/unmarshal-casm-class) - Load CASM class
