...
1. Calculating the transaction hash using [TransactionHashInvoke](./transaction-hash-invoke)
...
**Note**: Typically handled automatically by [BuildAndSendInvokeTxn](./build-and-send-invoke-txn), which manages transaction creation, signing, and submission in one call.
