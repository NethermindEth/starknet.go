...
func main() {
	...
	// Marshal to JSON (show first 1000 chars)
	casmJSON, err := json.MarshalIndent(casmClass, "", "  ")
	if err != nil {
		log.Fatal("Failed to marshal CASM:", err)
	}

	fmt.Printf("Compiled CASM (first 1000 chars):\n%s...\n", string(casmJSON[:1000]))
	fmt.Printf("\nTotal JSON size: %d bytes\n", len(casmJSON))
	fmt.Printf("Number of bytecode instructions: %d\n", len(casmClass.ByteCode))
}
