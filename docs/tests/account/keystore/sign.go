...
func main() {
	...
	// Verify the signature is deterministic (signing same message gives same signature)
	fmt.Println("\nVerifying deterministic signatures:")
	r2, s2, err := ks.Sign(ctx, pubKey1, msgHash)
	if err != nil {
		fmt.Printf("Error: %v\n", err)
		return
	}

	sameSig := r.String() == r2.String() && s.String() == s2.String()
	fmt.Printf("Same signature for same message: %v\n", sameSig)
