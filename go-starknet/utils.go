	...
	Subcommands: []*cli.Command{
		...
		},
		{
			...
			Action: func(cCtx *cli.Context) error {
				...
				publicKey, _, err := starknet.go.Curve.PrivateToPoint(privateKeyInt)
				if err != nil {
					return err
				}
		...
		},
		{
			...
			Action: func(cCtx *cli.Context) error {
				...
				x, y, err := starknet.go.Curve.Sign(messageInt, privateKeyInt)
				if err != nil {
					return err
				}
				fmt.Printf("public key:   0x%s\n", publicKey.Text(16))
		...
		},
		{
			...
			Action: func(cCtx *cli.Context) error {
				...
				y := starknet.go.Curve.GetYCoordinate(publicKeyInt)
				s0 := cCtx.String("s0")
				...
				ok = starknet.go.Curve.Verify(messageInt, s0Int, s1Int, publicKeyInt, y)
				if !ok {
