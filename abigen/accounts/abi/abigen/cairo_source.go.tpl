...
type {{$contract.Type}}{{.Normalized.Name}} struct {
	{{range $i, $key := .Normalized.Keys}}{{if $i}}, {{end}}{{$key.Name}} {{bindtype $key.Type}}{{end}}{{if and .Normalized.Keys .Normalized.Data}},{{end}}
	{{range $i, $data := .Normalized.Data}}{{if $i}}, {{end}}{{$data.Name}} {{bindtype $data.Type}}{{end}}{{if or .Normalized.Keys .Normalized.Data}},{{end}}
	Raw rpc.Event // Blockchain specific contextual infos
}

// Filter{{.Normalized.Name}} is a free log retrieval operation binding the contract event {{.Original.Name}}.
//
// Cairo: {{.Original.Name}}
