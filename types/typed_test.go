...
func (mail Mail) FmtDefinitionEncoding(field string) ([]felt.Felt, error) {
	fmtEnc := []felt.Felt{}
	if field == "from" {
		name, err := felt.UTF8StrToFelt(mail.From.Name)
		if err != nil {
			return nil, err
		}
		fmtEnc = append(fmtEnc, *name)
		fmtEnc = append(fmtEnc, felt.StrToFelt(mail.From.Wallet))
	} else if field == "to" {
		to, err := felt.UTF8StrToFelt(mail.To.Name)
		if err != nil {
			return nil, err
		}
		fmtEnc = append(fmtEnc, *to)
		fmtEnc = append(fmtEnc, felt.StrToFelt(mail.To.Wallet))
	} else if field == "contents" {
		content, err := felt.UTF8StrToFelt(mail.Contents)
		if err != nil {
			return nil, err
		}
		fmtEnc = append(fmtEnc, *content)
	}
	return fmtEnc, nil
}
...
func TestGetMessageHash(t *testing.T) {
	...
	hash, err := ttd.GetMessageHash(felt.StrToFelt("0xCD2a3d9F938E13CD947Ec05AbC7FE734Df8DD826"), mail, felt.GetCurve())
	if err != nil {
...
func BenchmarkGetMessageHash(b *testing.B) {
	...
	addr := felt.StrToFelt("0xCD2a3d9F938E13CD947Ec05AbC7FE734Df8DD826")
	b.Run(fmt.Sprintf("input_size_%d", addr.BitLen()), func(b *testing.B) {
		ttd.GetMessageHash(addr, mail, felt.GetCurve())
	})
}
...
func TestGetDomainHash(t *testing.T) {
	...
	hash, err := ttd.GetTypedMessageHash("StarkNetDomain", ttd.Domain, felt.GetCurve())
	if err != nil {
		t.Errorf("Could not hash message: %v\n", err)
	}
...
func TestGetTypedMessageHash(t *testing.T) {
	...
	hash, err := ttd.GetTypedMessageHash("Mail", mail, felt.GetCurve())
	if err != nil {
...
func TestGetSelectorFromName(t *testing.T) {
	sel1 := felt.GetSelectorFromName("initialize")
	sel2 := felt.GetSelectorFromName("mint")
	sel3 := felt.GetSelectorFromName("test")
	...
	if sel1.Hex() != exp1 || sel2.Hex() != exp2 || sel3.Hex() != exp3 {
		t.Errorf("invalid Keccak256 encoding: %v %v %v\n", sel1.Hex(), sel2.Hex(), sel3.Hex())
	}
}
